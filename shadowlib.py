...
# I made this library to share various
# utility functions across my modules.
# You can use this library in your own modules
# as well.

# P.S this library is still under development and may receive updates
# in the future.
...
class ShadowLib(loader.Library):
    """A library with various utility functions for Shadow modules."""

    developer = "@shadow_mod777"

    strings = {
        "name": "ShadowLib",
        "desc": "A library with various utility functions for Shadow modules.",
...
class UserUtils:
    ...
    async def get_info(
        ...
        userfull = await self._client(
            messages.GetFullUserRequest(id=user_id)
        )
...
class ChatUtils:
    ...
    async def get_info(self, chat) -> dict:
        try:
            chat_full = await self._client(
                messages.GetFullChannelRequest(channel=chat)
            )
    ...
    async def invite_bot(self, client, chat) -> bool:
        ...
        admin = AdminUtils(self._client, self)
...
class AdminUtils:
    ...
    async def set_role(self, chat, user, role_name, rank="Shadow Admin") -> bool:
    ...
    async def set_rights(self, chat, user, mask: int, rank: str = "Shadow Admin") -> bool:
...
class AdminRights(Rights):
    RIGHTS_LIST = [x for x in types.ChatAdminRights().to_dict().keys() if x != "_"]</content>
